...
# Testing performance for all linear models (lm1, lm2, and lm3)
```{r}
# Train/Test Split
...
```{r}
## Importing data and adding Viral 
onlinenews <- read.csv("https://raw.githubusercontent.com/jgscott/ECO395M/master/data/online_news.csv")
onlinenews$viral <- as.numeric(onlinenews$shares >= 1400)
...
colnames(onlinenews)
lm2 = lm(shares ~ num_hrefs+num_imgs + is_weekend + global_rate_negative_words + self_reference_avg_sharess + num_keywords + avg_negative_polarity + Category + num_videos , data=news_train)
...
glm1 = glm(viral ~ average_token_length + num_imgs  + num_videos + num_hrefs + weekday_is_saturday , data=onlinenews, family='binomial')
glm2 = glm(viral ~ num_hrefs +is_weekend + global_rate_negative_words + data_channel_is_bus + self_reference_avg_sharess + data_channel_is_world + data_channel_is_entertainment + num_keywords + avg_negative_polarity, data=onlinenews, family='binomial')
glm3 = glm(viral ~ (n_tokens_title + num_hrefs + weekday_is_monday + global_rate_positive_words + avg_negative_polarity + max_negative_polarity + is_weekend), data=onlinenews, family='binomial')
...
phat_train1 = predict(glm1, news_train) 
phat_train2 = predict(glm2, news_train) 
phat_train3 = predict(glm3, news_train) 
...
phat_test1 = predict(glm1, news_test)
phat_test2 = predict(glm2, news_test)
phat_test3 = predict(glm2, news_test)
